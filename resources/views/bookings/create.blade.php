            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('bookings.store') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="user_id" class="mt-1 block w-full rounded-md border-gray-300" required>
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}" @selected(old('user_id', auth()->id()) == $user->id)>{{ $user->full_name }} ({{ $user->email }})</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="house_id" class="mt-1 block w-full rounded-md border-gray-300" required>
                            ...
                            @foreach ($houses as $house)
                                <option value="{{ $house->id }}" @selected(old('house_id') == $house->id)>#{{ $house->house_number }} - {{ $house->house_type }} (Ksh {{ number_format($house->rent, 2) }})</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="booking_id" class="mt-1 block w-full rounded-md border-gray-300" required>
                            @foreach ($statuses as $status)
                                <option value="{{ $status->id }}" @selected(old('booking_id') == $status->id)>{{ $status->status }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex items-center gap-3">
                        <button class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700" type="submit">Save Booking</button>
